...
- **RESOLUTION**: NOT the last flagged day. `GAP_TOLERANCE_DAYS` is the maximum number of SKIPPED (unflagged) calendar days tolerated between two flagged days before they're considered separate episodes (see `episode/grouping.py`'s docstring for the precise definition and a worked example). An episode formally resolves once `GAP_TOLERANCE_DAYS` skipped days have elapsed after its last flagged day, confirmed by one further day (i.e. `end_day + GAP_TOLERANCE_DAYS + 1`) — or at the end of the merchant's available history, whichever comes first. In other words: resolution waits exactly as long as grouping would have waited to decide a *new* flagged day still belongs to this episode, then one day longer to confirm none arrived. This means an episode's `end_day` is always a few days later than its last `trigger_event`, by design — the system needs to see the quiet period actually hold before calling something resolved.
