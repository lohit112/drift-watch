...
`RESOLVED` is a structural event, not a confidence band: it fires when the
episode's flagged run has been quiet for `GAP_TOLERANCE_DAYS + 1` days (see
`episode/grouping.py`), or the merchant's available history ends. This is
deliberate — a low confidence score doesn't mean an episode is over, it
